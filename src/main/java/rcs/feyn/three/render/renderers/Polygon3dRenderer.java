...
public class Polygon3dRenderer {
  ...
  public static void render(Graphics3d graphics, Vector3d[] viewPortCoords, double intensity, int color) {
    render(graphics, viewPortCoords, new double[]{ intensity }, color);
  }
  ...
  public static void render(Graphics3d graphics, Vector3d[] viewPortCoords, double[] intensities, int color) {
    ...
    boolean gouraud = intensities.length == size;
    
    if (!gouraud) {
      color = ColorUtils.mulRGBA(color, intensities[0]);
    }
